...
- When updating GitHub actions (except `mio-19` actions), always update to the latest tag's commit hash, and include the tag name in a comment (e.g. `uses: actions/checkout@<hash> # <tag>`).
...
- **Default user aspects**: For Den host users, `users.<name>.aspect` defaults to `den.aspects.<name>` unless explicitly set. For example, `users.chester.classes = [ "homeManager" ];` implicitly uses `den.aspects.chester` and projects its `homeManager` branch into `home-manager.users.chester`. Override `users.<name>.aspect` only when a user should use a differently named aspect.
...
- **`hardenedPkg`**: This wrapper is used to preload `graphene-hardened-malloc` (`libhardened_malloc.so`). It has **nothing** to do with `firejail`. Do not confuse the two when sandboxing or debugging packages.
