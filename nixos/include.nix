    ...
    // (with upper; rec {
      ...
      script =
        let
          ...
          cmd = action: ''
            sudo ${config.systemd.package}/bin/systemd-inhibit env NIX_REMOTE=daemon ${lib.getExe pkgs.nixos-rebuild-ng} ${action} --flake . --log-format internal-json ${lib.optionalString config.config_impure "--impure "}"$@" |& ${pkgs.nix-output-monitor}/bin/nom --json
          '';
          upCmd = ''
            set -e
            if [ -d ~/Documents/config/nixos ]; then
              ...
              cd /Users/*/Documents/config/nixos
            fi
            ...
            if [ -d ../../config-public ]; then
              ...
              echo "[WARNING] ../../config-public does not exist" >&2
              git pull --no-edit https://github.com/mio-19/config-public.git
              nix flake update
              git add flake.lock
              git commit -m "nixos: lockup" || true
      ...
      */
      ...
      pkgs-chaotic-ff-nightly' = import inputs.chaotic-ff-nightly {
        config = config.nixpkgs.config // {
          cudaSupport = false;
          rocmSupport = false;
        };
      ...
      usualDMEnabledFor =
        cfg:
        cfg.services.displayManager.sddm.enable
        || cfg.services.displayManager.plasma-login-manager.enable
      ...
      # Legacy osConfig-based aliases (fine when the module does not set these options).
      kdeDMEnabled = kdeDMEnabledFor osConfig;
      qtIsPreferred = qtIsPreferredFor osConfig;
      usualDMEnabled = usualDMEnabledFor osConfig;
      atleastV3 = atleastV3For osConfig;
      atleastV4 = atleastV4For osConfig;
    });
in
this
// {
  scopeFor =
    ...
    this
    // {
      kdeDMEnabled = this.kdeDMEnabledFor cfg;
      usualDMEnabled = this.usualDMEnabledFor cfg;
      qtIsPreferred = this.qtIsPreferredFor cfg;
      atleastV3 = this.atleastV3For cfg;
      atleastV4 = this.atleastV4For cfg;
      hasAntigravity = this.hasAntigravityFor cfg;
      hasCursor = this.hasCursorFor cfg;
    };
}
