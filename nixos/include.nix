    ...
    // (with upper; rec {
      ...
      script =
        let
          ...
          cmd = action: ''
            sudo ${config.systemd.package}/bin/systemd-inhibit env NIX_REMOTE=daemon ${lib.getExe pkgs.nixos-rebuild-ng} ${action} --flake . --log-format internal-json --fallback ${lib.optionalString config.config_impure "--impure "}"$@" |& ${pkgs.nix-output-monitor}/bin/nom --json
          '';
          upCmd = ''
            set -e
