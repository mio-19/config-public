...
* **Suspend/Resume Service Restart** ([nixpkgs#432276](https://github.com/NixOS/nixpkgs/issues/432276)): Stop `fprintd` before sleep. On resume, `fprintd-resume` waits 2s for USB, restarts `fprintd`, then `pkill`s `kscreenlocker_greet` so Plasma respawns the greeter with a fresh PAM↔fprintd D-Bus session (restarting fprintd alone left a stale locker and caused intermittent fingerprint unlock). *(If it still fails, check `lsusb` for the Goodix device dropping off / USB autosuspend.)*
