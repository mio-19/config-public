...
3. **Bypass the portal and query dark mode from a cap-dropped subprocess.** The GUI inherently requires `cap_net_admin` to speak to the kernel Netlink API (to list tunnels). Without it, the GUI fails with `operation not permitted` when activating a row. Applying file capabilities breaks `xdg-desktop-portal` by setting the process `dumpable=0`. However, even if we forcefully set `dumpable=1` (via wrappers or CGO constructors), the Linux kernel's `ptrace_has_cap()` strict check *always* denies unprivileged processes (like `xdg-desktop-portal`) from reading `/proc/<pid>/root` if the target process retains elevated capabilities. Since `wireguird` must keep `cap_net_admin` to function, the GTK portal communication is fundamentally blocked. To solve this, we wrap the binary with a launcher script that sets `GTK_USE_PORTAL=0`. To preserve dark mode, the script first queries the portal's DBus color-scheme setting via a subprocess that intentionally drops all capabilities (`setpriv --ambient-caps=-all`), and passes the result to GTK via `GTK_THEME=Adwaita:dark`.
