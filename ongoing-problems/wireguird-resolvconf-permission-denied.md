...
3. **Inject a C constructor to run `prctl(PR_SET_DUMPABLE, 1)`** directly inside the `wireguird` Go program. The GUI inherently requires `cap_net_admin` to speak to the kernel Netlink API (to list tunnels). Without it, the GUI fails with `operation not permitted` when activating a row. Applying file capabilities breaks `xdg-desktop-portal` by setting the process `dumpable=0`. External C or bash wrappers that set dumpability fail because the Linux kernel resets `dumpable=0` across `execve` boundaries when transitioning capability states. By injecting a tiny C file into the Go codebase (`dumpable.go`) with an `__attribute__((constructor))`, the `prctl` syscall runs *after* `execve` as the very first thing in the final process, successfully preserving both capabilities and GTK portal access.
