...
3. **Restore the `makeWrapper` bash script with `prctl(PR_SET_DUMPABLE, 1)`** on the GUI binary. The GUI inherently requires `cap_net_admin` to speak to the kernel Netlink API (to list tunnels). Without it, the GUI fails with `operation not permitted` when activating a row. However, applying file capabilities breaks `xdg-desktop-portal` by setting the process `dumpable=0`. To fix this, we reverted a recent wrapper simplification and reinstated `dontWrapGApps = true` with a manual `makeWrapper` call, allowing a tiny C program to restore dumpability *after* the bash script executes, but before the Go program runs. This allows both the VPN to connect and GTK dark mode to function.
