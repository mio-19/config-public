      ...
      with (_include.scopeFor config);
      {
        ...
        environment.etc."nixos/lockscreen.jpg".source =
          pkgs.runCommand "lockscreen-jpg" { buildInputs = [ pkgs.imagemagick ]; }
            ''
              magick convert ${
                if config.hdr_very_bright then ../nixos/black.png else config.system_background
              } $out
            '';
        ...
        environment.systemPackages =
          ...
          ++ lib.optionals (config.services.displayManager.sddm.enable) [
            ...
            (pkgs.writeTextDir "share/sddm/themes/breeze/theme.conf.user" ''
              [General]
              background=${if config.hdr_very_bright then ../nixos/black.png else config.system_background}
            '')
          ]
          ++
            lib.optionals
        ...
        /*
          ...
          services.displayManager.plasma-login-manager.settings = {
            ...
            "Greeter/Wallpaper/org.kde.image/General" = {
              Image = "file://${if config.hdr_very_bright then ../nixos/black.png else config.system_background}";
        ...
        */
        ...
        qt = lib.mkIf kdeDMEnabled {
          ...
          enable = true;
          # https://github.com/NixOS/nixpkgs/blob/nixos-unstable/nixos/modules/config/qt.nix
          platformTheme = "kde";
          style = "breeze";
        };
